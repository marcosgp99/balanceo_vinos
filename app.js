...
const state = {
  ...
  planRawRows:   {},        // { "WO_NUM": rawRow[] } — para export
  planHeaders:   [],        // headers originales del plan — para export
  semanaMap:     {},
  expanded:      {},
  movedToN2:     {},
  filtroSemana:  'Todos',
...
function handleFileUpload(e, mode) {
  ...
  reader.onload = function(evt) {
    // SIN cellDates:true para evitar que números de WO se conviertan en fechas
...
function showColumnMapper(headers, detected, mode) {
  ...
  document.getElementById('modal-title').textContent   = titles[mode] || 'Mapear columnas';
...
function confirmImport() {
  ...
  const reqByMode = {
    plan:     ['codEq','semana','lts','wo'],
    entregas: ['codEq','semana','lts'],
    insumo:   ['wo','etiqueta','caja']
  };
  const newMap = {};
...
function processData(rows, m) {
  ...
  // Guardar headers y filas crudas para export (solo plan)
  ...
  return rows.map((r, i) => {
    ...
    // Forzar a string — nunca dejar que sea un objeto Date
    const wo = woRaw instanceof Date
      ? String(woRaw.getTime())
...
function parseExcelDate(val) {
  ...
  if (!s) return null;
  // Excel serial number (sin cellDates las fechas vienen como número)
...
function getSemaforoColor(woStr, semLabel) {
  ...
  const eOk  = isOk(ins.etiqueta);
  const cOk  = isOk(ins.caja);
  if (eOk && cOk)  return 'green';
  if (eOk || cOk)  return 'yellow';
  return 'red';
}
...
// Retorna la fecha de llegada más tardía (la problemática) para mostrar en la fila
function getArrivalInfo(woStr, semLabel) {
  ...
  const isOk = d  => !d || (semStart ? d <= semStart : true);
  const parts      = [];
  if (!isOk(ins.etiqueta)) parts.push('Etiq: ' + (ins.etiqueta ? fmtDateShort(ins.etiqueta) : '?'));
  if (!isOk(ins.caja))     parts.push('Caja: ' + (ins.caja     ? fmtDateShort(ins.caja)     : '?'));
  if (parts.length)        return { text: parts.join(' · '), ok: false };
  // Todo ok — mostrar fechas confirmadas si existen
  const ok = [];
  if (ins.etiqueta) ok.push('Etiq: ' + fmtDateShort(ins.etiqueta));
  if (ins.caja)     ok.push('Caja: ' + fmtDateShort(ins.caja));
  return ok.length ? { text: ok.join(' · '), ok: true } : { text: 'Sin fecha (OK)', ok: true };
}
...
function cargarDemoData() {
  ...
  state.insumoData = {
    ...
    '1134027': { etiqueta: null,                caja: new Date(2026,5,20) }, // amarillo N+3
  ...
  state.movedToN2     = {};
  state.expanded      = {};
  state.planHeaders   = ['Orden','CodEq','Semana','Lts','Estado','FeInicio','CantOrdenadaPT','ProductoDescripcion'];
  state.planRawRows   = {};
  state.planData.forEach(r => { state.planRawRows[r.wo] = [r.wo, r.codEq, r.semanaRaw, r.lts, r.estado, r.fecha, Math.round(r.lts/6/0.75), 'PRODUCTO DEMO']; });
  state.nextId = 60;
  ...
  updateStatus('insumo',    '3 WOs cargadas (demo)');
  buildSemanaMap();
  renderConfig();
  render();
}
...
function renderMetrics(pivot) {
  ...
  if (state.insumoLoaded) {
    state.planData.forEach(r => {
      const sem = getSemana(r);
      if (sem) {
        const c = getSemaforoColor(r.wo, sem);
        if (c === 'red' || c === 'yellow') insumoProb++;
      }
    });
  }
...
function renderSemanaFilter() {
  ...
  document.getElementById('semana-filter').innerHTML = pills.map(p => `
    <button class="fpill${p === 'N+3' ? ' n3' : ''}${p === state.filtroSemana ? ' on' : ''}"
      onclick="setFiltro('${p}')">
      ${p === 'Todos' ? 'Todas las semanas' : 'Semana ' + p}
    </button>`).join('');
}
...
function renderPivotTable(pivot) {
  ...
  // Ordenar: filtro primero, luego alfabético
  codigos.sort((a, b) => {
  ...
  const rows = codigos.map(cod => {
    ...
    const cells = semanas.map(s => {
      ...
      // Semáforo agregado por celda
      let semaHtml = '';
      if (state.insumoLoaded) {
        const colors = cell.wos.map(w => getSemaforoColor(w.wo, s));
        const counts = { green:0, yellow:0, red:0, none:0 };
        colors.forEach(c => counts[c]++);
        const agg = counts.red > 0 ? 'red' : counts.yellow > 0 ? 'yellow'
          : counts.none === colors.length ? 'none' : 'green';
        semaHtml = `<div class="cell-sema"><span class="sema-dot sema-${agg}"
          title="${counts.green}✓ ${counts.yellow}~ ${counts.red}✗"></span></div>`;
      }
      return `<td class="${cls}">
        <div class="cell-pct">${pTxt}</div>
        <div class="cell-vols">WO:${fmtLts(cell.planVol)}${state.entregaLoaded ? ' / E:' + fmtLts(cell.entregaVol) : ''}</div>
        ${semaHtml}
      </td>`;
    }).join('');
    ...
    const subRows = allWOs.map(w => {
      ...
      // Color de fila: solo para N+3
      let trCls = '';
      let arrivalHtml = '';
      if (isN3 && state.insumoLoaded) {
        const sColor = getSemaforoColor(w.wo, 'N+3');
        const canMove = sColor === 'green';
        trCls = canMove ? 'sub-ok' : 'sub-nok';
        const info = getArrivalInfo(w.wo, 'N+3');
        if (info) {
          arrivalHtml = `<span class="arrival-date${info.ok ? '' : ' nok'}">${info.text}</span>`;
        }
      }

      return `<tr class="${trCls}">
        <td style="padding-left:44px">
          ${isN3 && state.insumoLoaded
            ? `<span class="sema-dot sema-${getSemaforoColor(w.wo,'N+3')}" title="${getSemaforoText(w.wo,'N+3')}"></span> `
            : ''}
    ...
    <div class="pivot-wrap">
      ...
      <div class="pivot-legend">
        ...
        <span class="leg-item" style="color:#bbb;font-size:11px">
          % = WO Lts / Entrega &nbsp;·&nbsp; Lts incluye ${fmtLts(state.config.merma)} merma
        </span>
      </div>
    </div>`;
}
...
function showExportModal() {
  const movedIds = Object.keys(state.movedToN2).map(Number);
  if (!movedIds.length) {
    alert('No hay WOs movidas de N+3 a N+2 para exportar.\nMové al menos una WO usando el botón "↑ Mover a N+2".');
    return;
  }
  if (!state.planLoaded || !state.planHeaders.length) {
    alert('Primero importá el Plan.');
    return;
  }

  document.getElementById('modal-title').textContent = 'Exportar WOs balanceadas';
  document.getElementById('modal-desc').textContent  =
    `Se van a exportar ${movedIds.length} WO${movedIds.length > 1 ? 's' : ''} movidas de N+3 a N+2.\n¿Cuál columna del Plan es CantOrdenadaPT?`;
  document.getElementById('modal-confirm-btn').textContent = 'Exportar Excel';
  document.getElementById('modal-confirm-btn').onclick     = doExport;

  const autoDetectCant = state.planHeaders.find(h => /cantordenada/i.test(h)) || '';
  document.getElementById('col-map-fields').innerHTML = `
    <div class="col-map-row">
      <label>CantOrdenadaPT</label>
      <select id="map-export-cant">
        <option value="">— seleccionar —</option>
        ${state.planHeaders.map(h =>
          `<option value="${h}"${h === autoDetectCant ? ' selected' : ''}>${h}</option>`
        ).join('')}
      </select>
    </div>`;
  document.getElementById('modal-overlay').classList.remove('hidden');
...
function doExport() {
  ...
  const movedIds = new Set(Object.keys(state.movedToN2).map(Number));
  const wos      = state.planData.filter(r => movedIds.has(r.id));

  const wsData = [
    ['Orden', 'CodEq', 'Semana original', 'Semana nueva', 'Lts', 'Estado', 'Fecha inicio', 'CantOrdenadaPT'],
    ...wos.map(r => {
      const rawRow   = state.planRawRows[r.wo] || [];
      const cant     = cantIdx >= 0 ? rawRow[cantIdx] : '';
      const semOrig  = state.semanaMap[r.semanaRaw] || r.semanaRaw;
      return [r.wo, r.codEq, semOrig, 'N+2', r.lts, r.estado, r.fecha, cant];
    })
  ];
  ...
  // Ancho de columnas
  ws['!cols'] = [14,16,14,12,10,8,14,14].map(w => ({ wch: w }));
  XLSX.utils.book_append_sheet(wb, ws, 'WOs Balanceadas');
  XLSX.writeFile(wb, 'WOs_balanceadas_N+2.xlsx');
}
...
function renderAlertas(pivot) {
  ...
  if (a.surplus.length) {
    html += `<div class="alert-section">
      <div class="alert-section-title">&#128200; Sobrante en N+2 — adelantar N+3 (${a.surplus.length})</div>
      ${a.surplus.map(s => `<div class="alert-card surplus">
        <div class="alert-icon">&#128199;</div>
        <div style="flex:1">
          <div class="alert-title">${s.cod} — Sobrante: ${fmtLts(s.sobrante)} Lts en N+2</div>
          <div class="alert-sub">WOs de N+3 disponibles:</div>
          <div class="alert-wo-list">
            ${s.wosN3.map(w => {
              const sColor = state.insumoLoaded ? getSemaforoColor(w.wo, 'N+3') : 'none';
              const info   = state.insumoLoaded ? getArrivalInfo(w.wo, 'N+3') : null;
  ...
  if (a.insumoProb.length) {
    ...
    html += `<div class="alert-section">
      ...
      ${rojos.length ? `<div class="alert-card danger"><div class="alert-icon">&#128308;</div>
        <div><div class="alert-title">Sin insumos — no pueden producirse (${rojos.length})</div>
        ${rojos.map(x => `<div style="font-size:12px;margin-top:5px">
      ...
      ${amarillos.length ? `<div class="alert-card warn"><div class="alert-icon">&#128993;</div>
        <div><div class="alert-title">Insumos parciales (${amarillos.length})</div>
        ${amarillos.map(x => `<div style="font-size:12px;margin-top:5px">
  ...
  if (a.dups.length) {
    html += `<div class="alert-section">
      ...
      ${a.dups.map(d => `<div class="alert-card warn"><div class="alert-icon">&#128260;</div>
        ...
        <div class="alert-sub">Semanas: ${d.sems}</div></div></div>`).join('')}
    </div>`;
  }
  ...
  if (a.deficit.length) {
    html += `<div class="alert-section">
      ...
      ${a.deficit.map(d => `<div class="alert-card danger"><div class="alert-icon">&#128202;</div>
        ...
        <div class="alert-sub">WOs = ${d.pct}% de la entrega (max: ${state.config.maxPct}%)</div></div></div>`).join('')}
    </div>`;
  }

  if (!html) html = `<div class="alert-card ok"><div class="alert-icon">&#9989;</div>
    <div><div class="alert-title">Sin alertas activas</div>
    <div class="alert-sub">El plan esta correctamente balanceado.</div></div></div>`;
      ...
      <div class="config-grid">
        <div class="config-card">
          ...
          <div class="field-row">
            ...
            <span class="hint">Semanas de lunes a domingo · N+1 = +7 dias · N+2 = +14 · N+3 = +21</span>
          </div>
        </div>
        <div class="config-card">
          ...
          <div class="field-row">
            ...
            <span class="hint">Se suma al planVol de cada CodEq en cada semana</span>
          </div>
        </div>
      </div>
    </div>
...
function saveConfig() {
  ...
  setTimeout(() => { const el = document.getElementById('cfg-ok'); if (el) el.textContent = ''; }, 2500);
...
function fmtFecha(val) {
  ...
  const s = String(val).trim();
  // Excel serial sin cellDates
  const n = parseFloat(s);
  if (!isNaN(n) && n > 1000 && n < 100000) {
    return new Date((n - 25569) * 86400000).toLocaleDateString('es-AR');
  }
  return s.split(' ')[0];
}
